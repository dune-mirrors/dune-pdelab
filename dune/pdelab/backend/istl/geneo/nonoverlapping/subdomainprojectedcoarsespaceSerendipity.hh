...
#ifndef DUNE_PDELAB_BACKEND_ISTL_GENEO_NEWSUBDOMAINPROJECTEDCOARSESPACESERENDIPITY_HH
#define DUNE_PDELAB_BACKEND_ISTL_GENEO_NEWSUBDOMAINPROJECTEDCOARSESPACESERENDIPITY_HH
...
namespace Dune {
  namespace PDELab {
    ...
    class MultiVectorBundleSerendipity {
    public:
      typedef typename Vector::value_type value_type;

      MultiVectorBundleSerendipity(NonoverlappingOverlapAdapterSerendipity<GridView, GFS, Vector, Matrix>& adapter)
    ...
    template<typename V>
    struct MultiGatherScatterSerendipity
    {
    ...
    */
    ...
    class NonoverlappingSubdomainProjectedCoarseSpaceSerendipity : public CoarseSpace<X>
    {
    ...
    public:
      ...
      */
      NonoverlappingSubdomainProjectedCoarseSpaceSerendipity (NonoverlappingOverlapAdapterSerendipity<GridView, GFS, X, M>& adapter, const GridView& gridView, const M& AF_exterior_, std::shared_ptr<SubdomainBasis<X> > subdomainbasis, int verbosity = 1)
       : adapter_(adapter),
         gridView_(gridView),
    ...
    private:
      void setup_coarse_system() {
        ...
        MultiVectorBundleSerendipity<GridView, GFS, X, M> bundle(adapter_);
        ...
        for (rank_type basis_index_remote = 0; basis_index_remote < max_local_basis_size; basis_index_remote++) {
          ...
          communicator->forward<MultiGatherScatterSerendipity<MultiVectorBundleSerendipity<GridView, GFS, X, M>>>(bundle,bundle); // make function known in other subdomains
          if (verbosity_ > 2)
            bundle.print();
