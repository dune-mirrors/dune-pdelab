...
#ifndef DUNE_PDELAB_BACKEND_ISTL_GENEO_NEWSUBDOMAINPROJECTEDCOARSESPACE_SERENDIPITY_HELD_BY_RANK_0_HH
#define DUNE_PDELAB_BACKEND_ISTL_GENEO_NEWSUBDOMAINPROJECTEDCOARSESPACE_SERENDIPITY_HELD_BY_RANK_0_HH
...
namespace Dune {
  namespace PDELab {
    ...
    class MultiVectorBundleSerendipityHeldByRank0 {
    public:
      typedef typename Vector::value_type value_type;

      MultiVectorBundleSerendipityHeldByRank0(NonoverlappingOverlapAdapterSerendipity<GridView, GFS, Vector, Matrix>& adapter)
    ...
    template<typename V>
    struct MultiGatherScatterSerendipityHeldByRank0
    {
    ...
    */
    template<class GridView, typename GFS, class M, class X>
    class NonoverlappingSubdomainProjectedCoarseSpaceSerendipityHeldByRank0 : public CoarseSpace<X>
    {
    ...
    public:
      ...
      */
      NonoverlappingSubdomainProjectedCoarseSpaceSerendipityHeldByRank0 (NonoverlappingOverlapAdapterSerendipity<GridView, GFS, X, M>& adapter, const GridView& gridView, const M& AF_exterior_, std::shared_ptr<SubdomainBasis<X> > subdomainbasis, int verbosity = 1)
       : adapter_(adapter),
         gridView_(gridView),
    ...
    private:
      void setup_coarse_system() {
        ...
        MultiVectorBundleSerendipityHeldByRank0<GridView, GFS, X, M> bundle(adapter_);
        ...
        for (rank_type basis_index_remote = 0; basis_index_remote < max_local_basis_size; basis_index_remote++) {
          ...
          communicator->forward<MultiGatherScatterSerendipityHeldByRank0<MultiVectorBundleSerendipityHeldByRank0<GridView, GFS, X, M>>>(bundle,bundle); // make function known in other subdomains
          if (verbosity_ > 2)
            bundle.print();
    ...
    private:
      ...
      NonoverlappingOverlapAdapterSerendipity<GridView, GFS, X, M> adapter_;
