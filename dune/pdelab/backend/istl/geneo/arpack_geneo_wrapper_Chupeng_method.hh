...
#ifndef DUNE_FTWORTH_GENEO_ARPACK_GENEO_CHUPENG_METHOD_HH
#define DUNE_FTWORTH_GENEO_ARPACK_GENEO_CHUPENG_METHOD_HH
...
namespace ArpackMLGeneo
{
  ...
  class APP_BCRSMatMul_GeneralizedShiftInvertMode_Chupeng
  {
  public:
  ...
  public:
    ...
    APP_BCRSMatMul_GeneralizedShiftInvertMode_Chupeng (const BCRSMatrix& A,
                                                 const BCRSMatrix& A11,
                                                 const BCRSMatrix& A21,
                                                 const BCRSMatrix& B11,
                                                 const vector<int>& indice_map_BND,
                                                 const vector<int>& indice_map_INTERIOR)
      : A_(A), A21_(A21), A11_(A11), B11_(B11),
        indice_map_BND_(indice_map_BND), indice_map_INTERIOR_(indice_map_INTERIOR),
        A_solver(A_),
        // A_solver.setMatrix(A_),
        A11_solver(A11_),
        // A_LDL_solver(A_),
        a_(A11_.M() * mBlock), n_(A11_.N() * nBlock)
    {
      ...
      domainBlockVector.resize(A11_.N());
      rangeBlockVector.resize(A11_.M());
    }

    // inline void init () {
    //   A_solver.setMatrix<BCRSMatrix>(A_);
    // }

    //! Perform matrix-vector product w = A^{-1}*v for shift invert mode
    inline void multMvLHS (Real* v, Real* w)
    {
      // get vector v as an object of appropriate type
      arrayToDomainBlockVector(v,domainBlockVector);


      DomainBlockVector b_restricted(A21_.N());
      // for(int i=0; i<b_restricted.N(); i++)
      //   b_restricted[i] = 0.0;

      A21_.mv(domainBlockVector,b_restricted);

      DomainBlockVector b(A_.N()), a(A_.N());
      for(int i=0; i<b_restricted.N(); i++){
        b[indice_map_BND_[i]] = -b_restricted[i];
      }

      Dune::InverseOperatorResult result;
      A_solver.apply(a, b, result);

      DomainBlockVector a_restricted(A11_.N());
      // for(int i=0; i<a_restricted.N(); i++)
      //   a_restricted[i] = 0.0;

      for(int i=0; i<a_restricted.N(); i++){
        a_restricted[i] = a[indice_map_INTERIOR_[i]];
      }
      B11_.mv(a_restricted,rangeBlockVector);

      auto rangeBlockVector2 = rangeBlockVector;

      Dune::InverseOperatorResult result2;
      A11_solver.apply(rangeBlockVector2, rangeBlockVector, result2);

      // get vector w from object of appropriate type
      rangeBlockVectorToArray(rangeBlockVector2,w);
    };

    inline void multMvRHS (Real* v, Real* w)
    {
      // get vector v as an object of appropriate type
      arrayToDomainBlockVector(v,domainBlockVector);

      A11_.mv(domainBlockVector,rangeBlockVector);
    ...
    //   Dune::InverseOperatorResult result;
    //   A11_solver.apply(rangeBlockVector, domainBlockVector, result);
  ...
  protected:
    ...
    const BCRSMatrix& A_;
    const BCRSMatrix& A21_;
    const BCRSMatrix& A11_;
    const BCRSMatrix& B11_;

    // Dune::LDL<BCRSMatrix> A_solver;
    Dune::UMFPack<BCRSMatrix> A_solver;
    Dune::UMFPack<BCRSMatrix> A11_solver;
    // Dune::Cholmod<DomainBlockVector> A_solver;
    // Dune::LDL<BCRSMatrix> A_LDL_solver;

    const vector<int>& indice_map_BND_;
    const vector<int>& indice_map_INTERIOR_;
  ...
  class ArPackPlusPlus_Algorithms_Chupeng
  {
  public:
  ...
  public:
     ...
     */
    ArPackPlusPlus_Algorithms_Chupeng (const BCRSMatrix& A,
                               const BCRSMatrix& A11,
                               const BCRSMatrix& A21,
                               const BCRSMatrix& B11,
                               const vector<int>& indice_map_BND,
      ...
      : A_(A), A11_(A11), A21_(A21), B11_(B11), nIterationsMax_(nIterationsMax),
        indice_map_BND_(indice_map_BND), indice_map_INTERIOR_(indice_map_INTERIOR),
    ...
    inline void computeGenSymShiftInvertMinMagnitude (const Real& epsilon,
                                              ...
                                              std::vector<Real>& lambda) const
    {
      ...
      typedef APP_BCRSMatMul_GeneralizedShiftInvertMode_Chupeng<BCRSMatrix> WrappedMatrix;
      WrappedMatrix A(A_, A11_, A21_, B11_, indice_map_BND_, indice_map_INTERIOR_);
      ...
      ARSymGenEig<Real,WrappedMatrix,WrappedMatrix>
        dprob(nrows, nev, &A, &WrappedMatrix::multMvLHS, &A, &WrappedMatrix::multMvRHS, which, ncv, tol, maxit);
      ...
      // Final_transformation(x);
      Dune::UMFPack<BCRSMatrix> A_solver(A_);
      auto p = x;
      for(int iter_EV=0; iter_EV< p.size(); iter_EV++){

        BlockVector b_restricted(A21_.N());
        A21_.mv(p[iter_EV],b_restricted);
        BlockVector b(A_.N()), a(A_.N());
        for(int i=0; i<b_restricted.N(); i++){
          b[indice_map_BND_[i]] = -b_restricted[i];
        }
        Dune::InverseOperatorResult result;
    ...
    // void Final_transformation (std::vector<BlockVector>& x){
    //   Dune::UMFPack<BCRSMatrix> A_solver(A_);
