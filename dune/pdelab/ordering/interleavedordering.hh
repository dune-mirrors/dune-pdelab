  ...
  namespace PDELab {
    ...
    namespace interleaved_ordering {
      ...
      public:
         ...
         */
        Base(Node& node, bool container_blocked, const OrderingTag& ordering_tag, const std::shared_ptr<typename Base::GFSData>& gfs_data)
    ...
    public:
       ...
       */
      PowerInterleavedOrdering(bool container_blocked, const InterleavedOrderingTag& ordering_tag, const typename Node::NodeStorage& children, const std::shared_ptr<typename Base::GFSData>& gfs_data)
        : Node(children)
    ...
    struct power_gfs_to_interleaved_ordering_descriptor
    {
      ...
      static typename result<TC>::type transform(const GFS& gfs, const Transformation& t, const std::array<std::shared_ptr<TC>,TypeTree::StaticDegree<GFS>::value>& children)
      {
        ...
        return typename result<TC>::type(gfs.backend().blocked(gfs),gfs.orderingTag(),children,data);
      }
      ...
      {
        auto data = impl::gfs_data(const_cast<GFS&>(*gfs));
        return std::make_shared<typename result<TC>::type>(gfs->backend().blocked(*gfs),gfs->orderingTag(),children,data);
      }
    ...
    public:
       ...
       *       the leaf of the tree.
       */
      CompositeInterleavedOrdering(bool backend_blocked, const InterleavedOrderingTag& ordering_tag, const std::shared_ptr<typename Base::GFSData>& gfs_data, std::shared_ptr<Children>... children)
        : Node(children...)
    ...
    struct composite_gfs_to_interleaved_ordering_descriptor
    {
      ...
      {
        auto data = impl::gfs_data(const_cast<GFS&>(gfs));
        return typename result<TC...>::type(gfs.backend().blocked(gfs),gfs.orderingTag(),data,children...);
      }
      ...
      {
        auto data = impl::gfs_data(const_cast<GFS&>(*gfs));
        return std::make_shared<typename result<TC...>::type>(gfs->backend().blocked(*gfs),gfs.orderingTag(),data,children...);
      }
